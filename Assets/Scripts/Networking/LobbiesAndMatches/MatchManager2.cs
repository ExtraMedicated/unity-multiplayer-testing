...
public class MatchManager2 : NetworkBehaviour {
	public static MatchManager2 instance;
	...
	void AddPlayerToMatch(ExtNetworkRoomPlayer networkPlayer, string lobbyId){
		...
		if (matches.ContainsKey(lobbyId)){
			...
			if (match.isInProgress){
				networkPlayer.connectionToClient.Send(new AddPlayerToMatchError{errorText = "Game has already started."});
			} else if (match.players.Count >= match.maxPlayers){
				networkPlayer.connectionToClient.Send(new AddPlayerToMatchError{errorText = "Match is full."});
			} else {
			...
			networkPlayer.connectionToClient.Send(new AddPlayerToMatchError{errorText = "Match not found."});
		}
	...
	// TODO: Need to update the lobbies in the lobby list when adding/removing a player. Currently only happens within the lobby view.
	...
	public void RemovePlayerFromMatch(ExtNetworkRoomPlayer networkPlayer, string matchId){
		...
		if (matches.ContainsKey(networkPlayer.matchId)){
			...
			// matches[matchId] = match; // Triggers the update TODO: Shouldn't the RPC take care of this?
			if (match.players.Count == 0){
